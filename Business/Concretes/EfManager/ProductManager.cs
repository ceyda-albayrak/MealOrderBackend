...
namespace Business.Concretes
{
    public class ProductManager : IProductService
    {
        ...
        {
            var cid = _categoryDal.Get(p=>p.Id==product.CategoryId);
            var rid = _infoDal.Get(p=>p.Id==product.RestaurantId);
            if (cid!=null)
            {
                if (rid!=null)
                {
        ...
        {
            return new SuccessDataResult<Product>(_productDal.Get(p=>p.Id==id), Messages.Listed);
        }
        ...
        {
            return new SuccessDataResult<List<Product>>(_productDal.GetAll(p=>p.RestaurantId==id), Messages.Listed);
        }
