...
namespace Business.Concretes
{
    public class RestaurantInfoManager:  IRestaurantInfoService
    {
        ...
        {
            var rid = _regionDal.Get(p=>p.Id==info.RegionId);
            var uid = _userDal.Get(p=>p.Id==info.UserId);
            if (rid!=null)
            {
                if (uid!=null)
                {
        ...
        {
            return new SuccessDataResult<RestaurantInfo>(_infoDal.Get(p=>p.Id==id), Messages.Listed);
        }
